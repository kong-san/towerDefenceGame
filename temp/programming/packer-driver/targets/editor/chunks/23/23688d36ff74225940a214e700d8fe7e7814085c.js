          ...
          if (Collider.node.name == "Wei_Per") {
            this.schedule(function () {
              if (Collider.node.isValid) {
                ...
                if (that.currentLivingValue - harmedValue <= 0) {
                  that.node.parent.destroy();
                } else {
                  that.currentLivingValue -= harmedValue; // console.log("生命值为：",this.SanShePaoLivSysMag.livingValue)

                  that.node.parent.getChildByName("ProgressBar").getComponent(ProgressBar).progress = 0.5 * (that.currentLivingValue / that.livingValue);
                }
              }
            }, 5);
          }
...
//# sourceMappingURL=23688d36ff74225940a214e700d8fe7e7814085c.js.map