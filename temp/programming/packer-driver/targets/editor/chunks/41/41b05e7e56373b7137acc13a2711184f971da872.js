    ...
    execute: function () {
      ...
      _export("diji", diji = (_dec = ccclass('diji'), _dec2 = property(_crd && livingSystemManeger === void 0 ? (_reportPossibleCrUseOflivingSystemManeger({
        ...
        update(deltaTime) {
          ...
          let x = node.position.x; // if(x > 600){
          //     //敌军飞机不超过神木；
          // }else{
          ...
          for (let i = 0; i < this.bulletArray.length; i++) {
            ...
            let x = bulletNode.position.x; // if(this.targetArray.length>0){
        ...
        underFire(Collider) {
          ...
          let harmedValueFeng = find("Canvas/GlobalRoundControl").getComponent(_crd && combatConfigManeger === void 0 ? (_reportPossibleCrUseOfcombatConfigManeger({
            error: Error()
            ...
            if (!Prefab) {// console.log('子弹预制体为空')
              ...
              if (!node) {// console.log('没有节点')
                ...
                node.getComponent(_crd && BulletPerManeger === void 0 ? (_reportPossibleCrUseOfBulletPerManeger({
                  error: Error()
                }), BulletPerManeger) : BulletPerManeger).speed = that.DijiLivSysMag.attackSpeed; // let shit = pos.set(x,y)
                // x = shit.x - 800;                   
                // y = shit.y - 450;
                // console.log(x,y)
...
//# sourceMappingURL=41b05e7e56373b7137acc13a2711184f971da872.js.map