        ...
        start() {
          ...
          this.node.on(Node.EventType.TOUCH_MOVE, event => {
            ...
            if (this.node.parent.parent.parent.getChildByName('pao1')) {
              ...
              let currentNode = this.node.parent.parent.parent.getChildByName('pao1'); //event.touch._point获得的鼠标位置是相对画布左下角的，而当前的锚点是在正中心
              //TODO：增加防抖操作

              console.log(event.getLocationX(), event.getLocationY()); //触摸点的世界坐标
              // let pos = new Vec3(event.getLocationX(),event.getLocationY(),0);

              let pos = new Vec3(event.touch._point['x'], event.touch._point['y'], 0); // console.log('changabefore',pos)
              //转换为UI作表

              pos = currentNode.getComponent(UITransform).convertToNodeSpaceAR(pos); // let x = event.touch._point['x'];
...
//# sourceMappingURL=d5b4b467cf7d2087ccc74ba3048a0d65833a7401.js.map