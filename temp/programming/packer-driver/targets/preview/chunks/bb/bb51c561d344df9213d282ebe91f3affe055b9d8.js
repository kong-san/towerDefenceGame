System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Node, resources, Sprite, SpriteFrame, Layers, Label, Prefab, instantiate, _dec, _class, _class2, _descriptor, _crd, ccclass, property, infoManeger;
  ...
  return {
    ...
    execute: function () {
      ...
      __checkObsolete__(['_decorator', 'Component', 'Node', 'resources', 'Sprite', 'SpriteFrame', 'Layers', 'find', 'Label', 'Vec3', 'Prefab', 'instantiate']);
      ...
      _export("infoManeger", infoManeger = (_dec = ccclass('infoManeger'), _dec(_class = (_class2 = class infoManeger extends Component {
            ...
            if (this.currentType == 0) {
              ...
              resources.load('wenzikuang/spriteFrame', SpriteFrame, (_err, spriteFrame) => {
                ...
                resources.load("perfabs/infoLabel", Prefab, (err, Prefab) => {
                  if (!Prefab) {} else {
                    var nodeLabel = instantiate(Prefab);
              ...
              this.currentType = toolType; // this.refreshInfo(toolType,this.node.getChildByName("infoBoxsub"));
            } else {
        ...
        refreshInfo(toolType, node) {
          if (toolType == 1) {
            console.log("文字信息打印");
            console.log(node.getChildByPath("nodeLabel").getChildByName("name").getComponent(Label).string);
            node.getChildByPath("nodeLabel").getChildByName("name").getComponent(Label).string = this.SanShePaoInfo.name; // node.getChildByName("describeLabel").getComponent(Label).string = this.SanShePaoInfo.describe;
          }

          if (toolType == 2) {// node.getChildByName("nameLabel").getComponent(Label).string = this.ZhuiJiDanInfo.name;
            // node.getChildByName("describeLabel").getComponent(Label).string = this.ZhuiJiDanInfo.describe;
          }

          if (toolType == 3) {// node.getChildByName("nameLabel").getComponent(Label).string = this.GuangZiJianInfo.name;
...
//# sourceMappingURL=bb51c561d344df9213d282ebe91f3affe055b9d8.js.map