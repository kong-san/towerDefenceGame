      ...
      _export("PaoPerManeger", PaoPerManeger = (_dec = ccclass('PaoPerManeger'), _dec2 = property(_crd && livingSystemManeger === void 0 ? (_reportPossibleCrUseOflivingSystemManeger({
        ...
        start() {
          ...
          this.schedule(function () {
            ...
            if (that.currenTarget.isValid) {
              that.angle = that.getAngle(that.currenTarget.position, that.node.position);
            }
          }, 1); //注册单个碰撞体的回调函数
          ...
          resources.load("perfabs/PaoBulletPer", Prefab, (err, Prefab) => {
            if (!Prefab) {
              ...
              if (!node) {
                ...
                node.getComponent(_crd && BulletPerManeger === void 0 ? (_reportPossibleCrUseOfBulletPerManeger({
                  error: Error()
                }), BulletPerManeger) : BulletPerManeger).angle = that.angle; // let shit = pos.set(x,y)
                // x = shit.x - 800;                   
                // y = shit.y - 450;
        ...
        getAngle(start, end) {
          ...
          var angle = dir.signAngle(new Vec2(0, 1)); // let degreen = angle / Math.PI * 180 ;
...
//# sourceMappingURL=1b27c98c6a32f004b2eaabf750bb7f3fb3e5f4fe.js.map