        ...
        start() {
          ...
          this.node.on(Node.EventType.TOUCH_MOVE, event => {
            ...
            if (this.node.parent.parent.parent.getChildByName('pao1')) {
              console.log('炮节点已存在，正在移动'); //event.touch._point获得的鼠标位置是相对画布左下角的，而当前的锚点是在正中心
              //TODO：增加防抖操作

              console.log(event.getLocationX(), event.getLocationY()); //触摸点的世界坐标

              var pos = new Vec3(event.getLocationX(), event.getLocationY(), 0);
              console.log('changabefore', pos); //转换为UI作表

              pos = this.node.parent.parent.parent.getComponent(UITransform).convertToNodeSpaceAR(pos);
              console.log('changafter', pos); // let x = event.touch._point['x']-450;
              // let y = event.touch._point['y']-800;

              this.node.parent.parent.parent.getChildByName('pao1').setPosition(pos); // this.node.parent.parent.getChildByName('pao1').destroy();
            } else {
...
//# sourceMappingURL=144576b1e64409b77964e373f67228346843d8a2.js.map