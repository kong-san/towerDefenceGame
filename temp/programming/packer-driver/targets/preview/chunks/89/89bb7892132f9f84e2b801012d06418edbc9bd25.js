    ...
    execute: function () {
      ...
      _export("KongperManeger", KongperManeger = (_dec = ccclass('KongperManeger'), _dec(_class = class KongperManeger extends Component {
        ...
        onBeginContact(selfCollider, otherCollider, contact) {
          if (otherCollider.node.parent.name == "Pao_Pre") if (!this.arrived) {
            this.arrived = 1;
            var node = this.node.parent;
            var y = node.position.y;
            var x = node.position.x;
          ...
          this.schedule(() => {
            ...
            if (that.currentTarget.length) {
              ...
              if (that.currentTargetNode.isValid || that.currentTargetNode.name != "default") {
                var componentName = that.whichComponent(that.currentTargetNode.name); // console.log("敌战机锁定的我方目标是",that.currentTargetNode.getComponent(componentName));
...
//# sourceMappingURL=89bb7892132f9f84e2b801012d06418edbc9bd25.js.map