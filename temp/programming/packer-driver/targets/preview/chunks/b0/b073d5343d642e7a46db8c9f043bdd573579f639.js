      ...
      _export("selectToolManeger", selectToolManeger = (_dec = ccclass('selectToolManeger'), _dec(_class = (_class2 = class selectToolManeger extends Component {
        ...
        SelectToolAndMove(event) {
          if (find("Canvas/selectToolBox").getChildByName(this.toolname)) {
            ...
            var currentNode = this.node.parent.parent.parent.getChildByName(this.toolname);
            var pos = new Vec2();
        ...
        SelectToolAndCancel(event) {
          ...
          if (!(step == 2)) {
            ...
            var parentName = selectNode.getComponent(_crd && PaoState === void 0 ? (_reportPossibleCrUseOfPaoState({
              error: Error()
            }), PaoState) : PaoState).ToolStateManeger.master;
            console.log('这个node从属于', selectNode.getComponent(_crd && PaoState === void 0 ? (_reportPossibleCrUseOfPaoState({
              error: Error()
            }), PaoState) : PaoState).ToolStateManeger.master);
            console.log(selectNode.parent.getChildByName("left").getChildByName(parentName)); //TODO:增加逻辑；若该组件下面挂在了一个tool了，就不允许再创建新的tool；

            var NewNode = this.NewPao();
            NewNode.parent = this.node.parent.parent.parent.getChildByName("left").getChildByName(parentName);
            NewNode.active = true;
            selectNode.destroy(); // selectNode.setPosition(100,100,0)
...
//# sourceMappingURL=b073d5343d642e7a46db8c9f043bdd573579f639.js.map