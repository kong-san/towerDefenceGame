          ...
          this.schedule(() => {
            ...
            if (that.currentTarget.length) {
              ...
              for (var i = 0; i < that.currentTarget.length; i++) {
                //用链表来可能合适;
                if (that.currentTargetNode.name == "default" || !that.currentTargetNode.isValid || !that.currentTargetNode.name) {
              ...
              if (that.currentTargetNode.isValid && that.currentTargetNode.name != "default" && that.currentTargetNode) {
                var componentName = that.whichComponent(that.currentTargetNode.name);
                console.log("敌战机锁定的我方目标是", that.currentTargetNode.name);
...
//# sourceMappingURL=8b81e49b90b1e72c28f32ed8bf8db19cbf10d515.js.map